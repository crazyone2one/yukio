...
import { getProjectMemberOptions } from '/@/api/modules/project-management/project-member.ts'
...
export default function initOptionsFunc(
  ...
  if (
    ...
    type === UserRequestTypeEnum.SYSTEM_PROJECT
  ) {
  ...
  if (type === UserRequestTypeEnum.PROJECT_USER_GROUP) {
    // 项目-用户组
    return getProjectUserGroupOptions(
      params.projectId,
      params.userRoleId,
      params.keyword,
    )
