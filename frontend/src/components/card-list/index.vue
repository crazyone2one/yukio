...
import { computed, nextTick, onMounted, Ref, ref, watchEffect } from 'vue'
import { CommonList } from '/@/models/common.ts'
import useContainerShadow from '/@/hooks/use-container-shadow.ts'
    ...
    <div ref="msCardListContainerRef" :class="['ms-card-list-container', containerStatusClass]">
        ...
        >
            ...
            <slot v-if="isShow" name="empty">
                <n-empty :description="t('common.noData')" class="h-[200px] justify-center" />
            </slot>
        </div>
    </div>
</template>
