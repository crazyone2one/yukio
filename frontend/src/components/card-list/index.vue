...
import { CommonPage } from '/@/api/interface/common'
...
const props = withDefaults(
  defineProps<{
    ...
    remoteFunc?: (v: any) => Promise<CommonPage<any>> // 远程数据模式下，请求数据的函数
  }>(),
  {
    mode: 'static',
    gap: 24,
...
const isShow = computed(() => {
  ...
  return (
    !topLoading.value &&
    !bottomLoading.value &&
    (remoteList.value.length === 0 || props.list?.length === 0)
  )
})
/**
 * 初始化列表滚动监听
...
async function computedListSize(width?: number, height?: number) {
  ...
  const maxCols = Math.floor(
    (clientWidth + props.gap) / (props.cardMinWidth + props.gap),
  )
  // 最大行数 =（容器高度 + gap） / （最小卡片宽度 + gap）。因为卡片宽高是 1:1，所以行数和列数相同。这里+2是为了在视图区域外多加载两行，以避免滚动条不出现
  const maxRows =
    Math.round((clientHeight + props.gap) / (props.cardMinWidth + props.gap)) +
    2
...
<template>
  <div
    ref="msCardListContainerRef"
    :class="['ms-card-list-container', containerStatusClass]"
  >
    <div
      ref="msCardListRef"
      class="ms-card-list"
    ...
    >
      <div v-if="topLoading" class="ms-card-list-loading">
        <n-spin :show="topLoading"></n-spin>
      </div>
      <div
        v-for="(item, index) in props.mode === 'remote'
          ? remoteList
          : props.list"
        :key="item.key"
      ...
      <div v-if="bottomLoading" class="ms-card-list-loading">
        <n-spin :show="bottomLoading"></n-spin>
      </div>
    </div>
